...
impl DataProcessor {
    ...
    pub fn load_from_csv<P: AsRef<Path>>(&mut self, path: P) -> Result<usize, Box<dyn Error>> {
        ...
        for (line_num, line) in reader.lines().enumerate() {
            ...
            let value = match fields[1].parse::<f64>() {
                Ok(val) => val,
                Err(_) => continue,
            };
            ...
            let timestamp = match fields[3].parse::<u64>() {
                Ok(val) => val,
                Err(_) => continue,
            };
...
mod tests {
    ...
    fn test_load_from_csv() {
        ...
        writeln!(temp_file, "3,99.9,alpha,1625270400").unwrap();
...
mod tests {
    ...
    fn test_calculate_statistics() {
        ...
        let expected_variance = ((10.5 - expected_mean).powi(2) + 
                                (20.0 - expected_mean).powi(2) + 
                                (15.5 - expected_mean).powi(2)) / 3.0;
...
mod tests {
    ...
    fn test_csv_loading() {
        ...
        writeln!(temp_file, "1,Alice,25.5,true").unwrap();
        writeln!(temp_file, "2,Bob,30.0,false").unwrap();
        writeln!(temp_file, "3,Charlie,-5.0,true").unwrap();
        ...
        assert_eq!(processor.calculate_average(), 27.75);
    }
}