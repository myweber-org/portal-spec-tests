...
mod tests {
    ...
    fn test_env_substitution() {
        ...
        parser.load_from_str(config).unwrap();
        assert_eq!(parser.get("port"), Some(&"9090".to_string()));
        assert_eq!(parser.get("host"), Some(&"default_host".to_string()));
    }
}use std::collections::HashMap;
use std::fs;
use std::path::Path;
...
mod tests {
    ...
    fn test_validation() {
        let config = Config::new();
        let result = config.validate_required(&["timeout", "missing_key"]);
        
        assert!(result.is_err());
        let errors = result.unwrap_err();
        assert!(errors.iter().any(|e| e.contains("missing_key")));
    }
}