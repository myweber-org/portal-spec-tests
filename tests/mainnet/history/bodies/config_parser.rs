...
mod tests {
    ...
    #[test]
    fn test_env_substitution() {
        env::set_var("CUSTOM_PATH", "/usr/local/bin");
        
        let mut file = NamedTempFile::new().unwrap();
        writeln!(file, "PATH=${CUSTOM_PATH}").unwrap();
        writeln!(file, "STATIC_PATH=/static").unwrap();

        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.get("PATH"), Some(&"/usr/local/bin".to_string()));
        assert_eq!(config.get("STATIC_PATH"), Some(&"/static".to_string()));
    }

    #[test]
    fn test_get_or_default() {
        let mut file = NamedTempFile::new().unwrap();
        writeln!(file, "EXISTING_KEY=value1").unwrap();

        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.get_or_default("EXISTING_KEY", "default"), "value1");
