...
impl CsvParser {
    ...
    pub fn parse_string(&self, content: &str) -> Vec<Vec<String>> {
        content
            ...
            .filter(|(idx, _)| !(self.has_header && *idx == 0))
            .map(|(_, line)| {
...
mod tests {
    ...
    #[test]
    fn test_parse_string_with_empty_lines() {
        let parser = CsvParser::new(',', false);
        let data = "John,30,New York\n\nJane,25,London\n";
        let result = parser.parse_string(data);
...
mod tests {
    ...
    fn test_empty_fields() {
        ...
        assert_eq!(result[0], vec!["field1", "field2", ""]);
        assert_eq!(result[1], vec!["value1", "", "value3"]);
    }
}use std::error::Error;
use std::fs::File;
use std::io::{BufRead, BufReader};
...
mod tests {
    ...
    fn test_parse_file() -> Result<(), Box<dyn Error>> {
        ...
        assert_eq!(result[1], vec!["2", "Item B", "200.0"]);
