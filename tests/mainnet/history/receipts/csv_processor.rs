...
impl CsvProcessor {
    ...
    pub fn load_from_file<P: AsRef<Path>>(&mut self, path: P) -> Result<(), CsvError> {
        ...
        for (line_num, line) in reader.lines().enumerate() {
            ...
            if parts.len() != 4 {
                return Err(CsvError::ParseError(
                    format!("Line {}: expected 4 fields, found {}", line_num + 1, parts.len())
                ));
            }
...
mod tests {
    ...
    fn test_csv_processing() {
        ...
        writeln!(csv_data, "3,Charlie,19.2,true").unwrap();
        ...
        let record = processor.find_by_name("Bob");
        assert!(record.is_some());
        assert_eq!(record.unwrap().id, 2);
    }
}