...
mod tests {
    ...
    fn test_aggregate_by_category() {
        ...
        let cat1_data = aggregated.iter().find(|d| d.category == "Category1").unwrap();
        assert_eq!(cat1_data.total_value, 300.0);
        assert_eq!(cat1_data.average_value, 150.0);
        assert_eq!(cat1_data.record_count, 2);
    }

    #[test]
    fn test_write_aggregated_data() -> Result<(), Box<dyn Error>> {
        let aggregated_data = vec![AggregatedData {
        ...
        write_aggregated_data(&aggregated_data, temp_path)?;

        let file_content = std::fs::read_to_string(temp_path)?;
        assert!(file_content.contains("TestCategory"));
        assert!(file_content.contains("500"));
