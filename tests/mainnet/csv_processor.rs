...
impl CsvProcessor {
    ...
    pub fn get_statistics(&self) -> (usize, f64, f64, usize) {
        let count = self.records.len();
        let avg_value = if count > 0 {
            self.total_value / count as f64
        } else {
            0.0
        };
        
        (count, self.total_value, avg_value, self.active_count)
    }

    pub fn filter_by_value(&self, min_value: f64) -> Vec<&CsvRecord> {
        self.records
            .iter()
