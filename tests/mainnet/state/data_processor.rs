...
mod tests {
    ...
    fn test_csv_loading() {
        ...
        csv_content.push_str("1,10.5,A\n");
        csv_content.push_str("2,15.3,B\n");
        csv_content.push_str("3,8.7,A\n");
    ...
    fn test_filtering() {
        ...
        let category_c = processor.filter_by_category("C");
        assert_eq!(category_c.len(), 0);
    }
    ...
    fn test_statistics() {
        ...
        let (min, max, avg) = empty_processor.get_statistics();
        assert_eq!(min, 0.0);
        assert_eq!(max, 0.0);
        assert_eq!(avg, 0.0);
    }
...
impl DataProcessor {
    ...
    pub fn filter_range(&mut self, min: f64, max: f64) {
        self.data.retain(|&x| x >= min && x <= max);
    }
...
mod tests {
    ...
    fn test_data_processing() {
        ...
        let original_count = processor.data_count();
        processor.filter_range(15.0, 30.0);
        assert!(processor.data_count() < original_count);
    }
}