...
mod tests {
    ...
    fn test_data_processor() {
        ...
        writeln!(temp_file, "1,45.5,category_a").unwrap();
        writeln!(temp_file, "2,120.3,category_b").unwrap();
        writeln!(temp_file, "3,890.7,category_a").unwrap();
        ...
        let avg = processor.calculate_average();
        assert!(avg.is_some());
        assert!((avg.unwrap() - 352.166666).abs() < 0.001);
