...
mod tests {
    ...
    fn test_calculate_statistics() {
        ...
        let avg_column_1 = processor.calculate_statistics(&records, 1).unwrap();
        assert!((avg_column_1 - 30.0).abs() < 0.001);
    }
}
use std::error::Error;
use std::fs::File;
...
impl DataProcessor {
    ...
    fn normalize_data(&self, data: &[f64]) -> Result<Vec<f64>, ValidationError> {
        ...
        if max_value <= 0.0 {
            return Err(ValidationError {
                message: "All data values must be positive for normalization".to_string(),
            });
        }
