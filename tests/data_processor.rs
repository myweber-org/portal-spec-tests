...
mod tests {
    ...
    fn test_get_column() {
        ...
        writeln!(temp_file, "10;20;30").unwrap();
        writeln!(temp_file, "40;50;60").unwrap();
        
        let processor = DataProcessor::new(temp_file.path().to_str().unwrap(), ';');
        let column = processor.get_column(1).unwrap();
        
        assert_eq!(column, vec!["20", "50"]);
    }
}
use std::error::Error;
use std::fs::File;
...
mod tests {
    ...
    fn test_calculate_statistics() {
        ...
        assert!((std_dev - 2.054804667).abs() < 1e-6);
    }
}
use std::error::Error;
use std::fs::File;
...
mod tests {
    ...
    fn test_csv_operations() {
        let mut processor = DataProcessor::new();
        processor.add_record(DataRecord::new(1, "Test".to_string(), 15.5, "Demo".to_string()));
