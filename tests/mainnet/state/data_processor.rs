...
impl DataProcessor {
    ...
    pub fn load_from_csv<P: AsRef<Path>>(&mut self, path: P) -> Result<usize, Box<dyn Error>> {
        ...
        for (line_num, line) in reader.lines().enumerate() {
            ...
            let record = match DataRecord::new(id, value, parts[2]) {
                Ok(record) => record,
                Err(_) => continue,
            };
...
mod tests {
    ...
    fn test_csv_loading() {
        ...
        writeln!(file, "invalid,data,here").unwrap();
...
impl DataProcessor {
    ...
    pub fn process_data(&self, input: &[f64]) -> Result<Vec<f64>, ValidationError> {
        ...
        if max_value.abs() < f64::EPSILON {
            return Ok(vec![0.0; normalized.len()]);
        }
    ...
    pub fn calculate_statistics(&self, data: &[f64]) -> Result<(f64, f64, f64), ValidationError> {
        ...
        if valid_data.is_empty() {
            return Err(ValidationError {
                message: "No valid data points available".to_string(),
            });
        }
