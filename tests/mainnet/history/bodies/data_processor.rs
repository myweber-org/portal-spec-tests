...
pub fn validate_numeric_data(data: &[f64]) -> Result<(), ValidationError> {
    ...
    for &value in data {
        if value.is_nan() || value.is_infinite() {
            return Err(ValidationError::new("Data contains NaN or infinite values"));
        }
    }
...
mod tests {
    ...
    fn test_validate_numeric_data() {
        ...
        assert!(validate_numeric_data(&empty_data).is_err());
    }
    ...
    fn test_normalize_data() {
        ...
        let constant_data = vec![7.0, 7.0, 7.0];
        let normalized_constant = normalize_data(&constant_data);
