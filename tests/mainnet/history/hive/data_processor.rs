...
impl DataProcessor {
    ...
    pub fn process_records(&mut self) -> Vec<DataRecord> {
        ...
        for record in &self.records {
            ...
            if record.category == "special" {
                if let Ok(_) = processed_record.transform_value(1.5) {
...
mod tests {
    ...
    fn test_invalid_id() {
        ...
        assert!(matches!(record.err(), Some(ValidationError::InvalidId)));
    }
    ...
    fn test_invalid_value() {
        ...
        assert!(record.is_err());
        assert!(matches!(record.err(), Some(ValidationError::InvalidValue)));
    }
    ...
    fn test_empty_category() {
        ...
        assert!(record.is_err());
        assert!(matches!(record.err(), Some(ValidationError::EmptyCategory)));
    }
    ...
    fn test_invalid_transformation() {
        let mut record = DataRecord::new(1, 800.0, "test".to_string()).unwrap();
        assert!(record.transform_value(2.0).is_err());
    }
}
