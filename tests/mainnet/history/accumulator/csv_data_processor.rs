...
pub fn process_csv_data(file_path: &str) -> Result<(), Box<dyn Error>> {
    ...
    println!("Unique categories: {}", categories.join(", "));
    
    for category in categories {
        let category_records = processor.filter_by_category(&category);
        let category_total: f64 = category_records.iter().map(|r| r.value).sum();
        println!("Category '{}': {} records, total value: {:.2}", 
                 category, category_records.len(), category_total);
    }
