...
impl JsonParser {
    ...
    fn parse_escape_sequence(&mut self) -> Result<char, ParseError> {
        match self.peek_char() {
            ...
            Some('u') => {
                self.advance();
                let hex_code = self.read_n_chars(4)?;
    ...
    fn expect(&mut self, expected: &str) -> Result<(), ParseError> {
        ...
        for (i, &expected_char) in chars.iter().enumerate() {
            match self.input.get(self.position + i) {
                ...
                Some(&actual_char) => return Err(ParseError::UnexpectedCharacter(
                    actual_char,
                    self.position + i,
                )),
                None => return Err(ParseError::UnexpectedEndOfInput),
    ...
    fn read_n_chars(&mut self, n: usize) -> Result<String, ParseError> {
        ...
        let result: String = self.input[self.position..self.position + n].iter().collect();
        self.position += n;
        Ok(result)
    }
}
...
pub fn pretty_print_json(json: &JsonValue, indent: usize) -> String {
    fn pretty_print_helper(json: &JsonValue, indent_level: usize, spaces: usize) -> String {
        ...
        let inner_indent = " ".repeat((indent_level + 1) * spaces);
        ...
        match json {
            ...
            JsonValue::Array(arr) => {
                ...
                let items: Vec<String> = arr.iter()
                    .map(|item| format!("{}{}", inner_indent, pretty_print_helper(item, indent_level + 1, spaces)))
                    .collect();
                format!("[\n{}\n{}]", items.join(",\n"), indent_str)
            }
            JsonValue::Object(obj) => {
                ...
                let items: Vec<String> = obj.iter()
                    .map(|(key, value)| {
                        format!("{}\"{}\": {}", inner_indent, key, pretty_print_helper(value, indent_level + 1, spaces))
                    })
                    .collect();
                format!("{{\n{}\n{}}}", items.join(",\n"), indent_str)
            }
