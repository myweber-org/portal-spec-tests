...
mod tests {
    ...
    fn test_filter_by_category() {
        ...
        let type_b_records = processor.filter_by_category("TypeB");
        assert_eq!(type_b_records.len(), 1);
    }
    ...
    fn test_find_max_value() {
        ...
        writeln!(temp_file, "id,value,category").unwrap();
        writeln!(temp_file, "1,10.5,TypeA").unwrap();
        writeln!(temp_file, "2,25.3,TypeB").unwrap();
        writeln!(temp_file, "3,15.7,TypeA").unwrap();
        ...
        assert_eq!(max_record.unwrap().value, 25.3);
    }
}
use std::error::Error;
use std::fs::File;
