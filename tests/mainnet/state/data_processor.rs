...
mod tests {
    ...
    fn test_filter_numeric_greater_than() {
        ...
        let filtered = DataProcessor::filter_numeric_greater_than(&records, 1, 10.0);

        assert_eq!(filtered.len(), 2);
        assert!(filtered.iter().any(|r| r[0] == "A"));
        assert!(filtered.iter().any(|r| r[0] == "C"));
        assert!(!filtered.iter().any(|r| r[0] == "B"));
    }
}
use std::error::Error;
use std::fs::File;
...
mod tests {
    ...
    fn test_csv_loading() {
        ...
        writeln!(temp_file, "2,75.0,TypeB").unwrap();
        writeln!(temp_file, "3,110.0,TypeC").unwrap();
    ...
    fn test_average_calculation() {
        ...
        processor.records.push(DataRecord::new(2, 70.0, "B"));
        processor.records.push(DataRecord::new(3, 90.0, "C"));
