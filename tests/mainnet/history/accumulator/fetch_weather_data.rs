...
mod tests {
    ...
    async fn test_weather_fetching() {
        let _m = mock("GET", "/v1/current?city=London&key=test_key")
            .with_status(200)
            .with_header("content-type", "application/json")
            .with_body(r#"{"temperature": 15.5, "humidity": 65.0, "description": "Partly cloudy", "wind_speed": 12.3}"#)
            .create();
...
pub async fn get_weather(api_key: &str, city: &str) -> Result<String, reqwest::Error> {
    let url = format!(
        "https://api.openweathermap.org/data/2.5/weather?q={}&appid={}&units=metric",
        city, api_key
    );
