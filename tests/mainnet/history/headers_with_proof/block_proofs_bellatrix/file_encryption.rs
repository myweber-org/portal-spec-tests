...
mod tests {
    ...
    fn test_file_encryption() -> io::Result<()> {
        ...
        let test_data = b"Test data for encryption demonstration";
        fs::write(temp_input.path(), test_data)?;

        let mut cipher = XorCipher::new("test_key_123");
        cipher.encrypt_file(temp_input.path(), temp_encrypted.path())?;
        
        let mut cipher2 = XorCipher::new("test_key_123");
        cipher2.decrypt_file(temp_encrypted.path(), temp_decrypted.path())?;
