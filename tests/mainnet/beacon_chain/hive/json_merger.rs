...
mod tests {
    ...
    fn test_deep_merge() {
        ...
        assert_eq!(base["b"]["inner"], json!(2));
        assert_eq!(base["b"]["new"], json!(3));
    }
}use std::collections::HashMap;
use std::fs;
use std::path::Path;
...
fn resolve_conflict(key: &str, existing: &Value, new: &Value) -> Value {
    match (existing, new) {
        ...
        _ => {
            eprintln!("Conflict for key '{}': {} vs {}", key, existing, new);
            new.clone()
        }
    }
}use serde_json::{Value, Map};
...
fn main() {
    ...
    for input_path in &args[2..] {
        ...
        if let Value::Object(map) = json_value {
            ...
            eprintln!("File {} does not contain a JSON object", input_path);
            process::exit(1);
        }
    }
