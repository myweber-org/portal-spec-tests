...
impl Config {
    ...
    fn process_value(input: &str) -> String {
        let mut result = String::new();
...
mod tests {
    ...
    fn test_env_substitution() {
        ...
        writeln!(file, "UNSET_VAR=${NOT_SET}").unwrap();

        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.get("DATABASE_URL"), Some(&"postgres://admin:secret@localhost/db".to_string()));
        assert_eq!(config.get("MODE"), Some(&"production".to_string()));
        assert_eq!(config.get("UNSET_VAR"), Some(&"${NOT_SET}".to_string()));
    }
}