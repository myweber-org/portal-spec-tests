...
mod tests {
    ...
    fn test_file_encryption() -> io::Result<()> {
        ...
        process_file(temp_input.path(), temp_output.path(), key)?;

        let mut encrypted_data = Vec::new();
        fs::File::open(temp_output.path())?.read_to_end(&mut encrypted_data)?;
        assert_ne!(encrypted_data, test_data);

        let mut temp_decrypted = NamedTempFile::new()?;
        process_file(temp_output.path(), temp_decrypted.path(), key)?;

        let mut decrypted_data = Vec::new();
        fs::File::open(temp_decrypted.path())?.read_to_end(&mut decrypted_data)?;
        assert_eq!(decrypted_data, test_data);
