...
impl DataProcessor {
    ...
    pub fn generate_summary(&self) -> String {
        ...
        if let Some(median) = self.calculate_median() {
            summary.push_str(&format!("Median: {:.4}\n", median));
        }
        
        if let Some(std_dev) = self.calculate_standard_deviation() {
            summary.push_str(&format!("Standard Deviation: {:.4}\n", std_dev));
        }
        
        summary.push_str("Category frequencies:\n");
...
mod tests {
    ...
    fn test_data_processor() {
        ...
        writeln!(temp_file, "B,25.1").unwrap();
        
        let result = processor.load_from_csv(temp_file.path().to_str().unwrap());
        assert!(result.is_ok());
        
        assert_eq!(processor.data.len(), 5);
        assert_eq!(processor.calculate_mean().unwrap(), 16.1);
        assert_eq!(processor.calculate_median().unwrap(), 15.7);
...
mod tests {
    ...
    fn test_csv_parsing() -> Result<(), Box<dyn Error>> {
        let mut temp_file = NamedTempFile::new()?;
        writeln!(temp_file, "1.5,2.7,3.2\n4.1,5.9,6.3")?;
        
        let ds = DataSet::from_csv(temp_file.path())?;
        assert_eq!(ds.count(), 6);
        assert_eq!(ds.mean(), Some((1.5+2.7+3.2+4.1+5.9+6.3)/6.0));
