...
fn handle_transport_protocol(
    ...
    match protocol {
        ...
        _ => {
            ...
            println!("Other Protocol: {} -> {} [Protocol: {}]", source, destination, protocol);
        }
    }
}
            ...
            Ok(packet) => {
                if let Some(ethernet_packet) = EthernetPacket::new(packet) {
                    match ethernet_packet.get_ethertype() {
                        ...
                        EtherTypes::Ipv6 => {
                            stats.update("IPv6", "Unknown", "Unknown");
                            println!("IPv6 Packet captured");
                        }
                        EtherTypes::Arp => {
                            stats.update("ARP", "Unknown", "Unknown");
                            println!("ARP Packet captured");
                        }
                        _ => {
                            stats.update("Other-EtherType", "Unknown", "Unknown");
                        }
                    }
                }
