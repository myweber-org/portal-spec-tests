...
mod tests {
    ...
    fn test_get_parsed() {
        ...
        let result: Result<u32, _> = config.get_parsed("missing");
        assert!(result.is_err());
    }
}use std::collections::HashMap;
use std::env;
...
mod tests {
    ...
    fn test_basic_parsing() {
        ...
        writeln!(file, "PORT=8080").unwrap();
        writeln!(file, "# This is a comment").unwrap();
        writeln!(file, "  KEY_WITH_SPACES  =  some value  ").unwrap();
        
        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.get("DATABASE_URL"), Some(&"postgres://localhost/db".to_string()));
        assert_eq!(config.get("PORT"), Some(&"8080".to_string()));
        assert_eq!(config.get("KEY_WITH_SPACES"), Some(&"some value".to_string()));
