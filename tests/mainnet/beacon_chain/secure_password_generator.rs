...
impl PasswordGenerator {
    ...
    pub fn validate_strength(password: &str) -> (bool, HashSet<&'static str>) {
        ...
        if password.len() < 12 {
            issues.insert("Password should be at least 12 characters long");
        }
        ...
        if !has_lowercase {
            issues.insert("Password should contain lowercase letters");
        }
        if !has_uppercase {
            issues.insert("Password should contain uppercase letters");
        }
        if !has_digit {
            issues.insert("Password should contain digits");
        }
        if !has_special {
            issues.insert("Password should contain special characters");
        }
...
mod tests {
    ...
    fn test_validation() {
        ...
        let (is_strong, _) = PasswordGenerator::validate_strength(strong_password);
        assert!(is_strong);

        let weak_password = "weak";
        let (is_strong, issues) = PasswordGenerator::validate_strength(weak_password);
        assert!(!is_strong);
        assert!(issues.contains("Password should be at least 12 characters long"));
    }
}