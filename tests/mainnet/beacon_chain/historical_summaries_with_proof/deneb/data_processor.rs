...
impl DataRecord {
    ...
    pub fn normalize(&mut self) -> Result<(), ProcessingError> {
        ...
        if max - min < f64::EPSILON {
            return Err(ProcessingError::TransformationError(
                "Cannot normalize constant values".to_string()
            ));
        }
    ...
    pub fn calculate_statistics(&self) -> HashMap<String, f64> {
        ...
        stats.insert("variance".to_string(), variance);
        stats.insert("min".to_string(), self.values.iter().copied().fold(f64::INFINITY, f64::min));
        stats.insert("max".to_string(), self.values.iter().copied().fold(f64::NEG_INFINITY, f64::max));
        stats.insert("sum".to_string(), sum);
...
mod tests {
    ...
    fn test_statistics_calculation() {
        ...
        assert!((stats["mean"] - 2.5).abs() < 0.001);
        assert!((stats["variance"] - 1.25).abs() < 0.001);
        ...
        if let Some(header_result) = lines.next() {
            ...
            for line_result in lines {
                ...
                for (i, column) in columns.iter().enumerate() {
                    if let Ok(value) = values[i].parse::<f64>() {
...
mod tests {
    ...
    fn test_data_processor() {
        ...
        assert!((mean - 11.466666666666666).abs() < 0.0001);
        assert!((std_dev - 2.7495454169735).abs() < 0.0001);
