...
mod tests {
    ...
    fn test_env_substitution() {
        ...
        writeln!(file, "HOST=server-${APP_SECRET}.com").unwrap();

        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.get("SECRET_KEY"), Some(&"my_secret_value".to_string()));
        assert_eq!(config.get("HOST"), Some(&"server-my_secret_value.com".to_string()));
    }
}
use std::collections::HashMap;
use std::env;
...
mod tests {
    ...
    #[test]
    fn test_env_substitution() {
        env::set_var("SECRET_PASSWORD", "env_password");
        ...
        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.get("PASSWORD"), Some(&"env_password".to_string()));
        assert_eq!(config.get("NORMAL_VALUE"), Some(&"static".to_string()));
        
        env::remove_var("SECRET_PASSWORD");
    }
