...
mod tests {
    ...
    fn test_data_processing() {
        ...
        let mut temp_file = NamedTempFile::new().unwrap();
        writeln!(temp_file, "12.5,apple,8.3,banana,apple").unwrap();
        writeln!(temp_file, "15.2,orange,banana,7.1").unwrap();
        
        let result = processor.load_csv(temp_file.path().to_str().unwrap());
        assert!(result.is_ok());
        
        assert_eq!(processor.calculate_mean(), Some(10.775));
        assert_eq!(processor.calculate_median(), Some(9.9));
        
        let top_categories = processor.get_top_categories(2);
        assert_eq!(top_categories.len(), 2);
        assert_eq!(top_categories[0], ("banana".to_string(), 2));
    }
}
use std::error::Error;
use std::fs::File;
...
mod tests {
    ...
    fn test_data_processing() {
        ...
        assert!(processor.get_summary().contains("Data points: 5"));
    }
}