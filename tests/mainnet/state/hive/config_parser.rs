...
mod tests {
    ...
    fn test_env_substitution() {
        ...
        writeln!(file, "SIMPLE=$NONEXISTENT").unwrap();

        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.get("DB_URL"), Some(&"postgres://admin:secret123@localhost".to_string()));
        assert_eq!(config.get("SIMPLE"), Some(&"".to_string()));
    }
}use std::collections::HashMap;
use std::env;
use std::fs;
...
mod tests {
    ...
    fn test_env_substitution() {
        ...
        writeln!(file, "API_KEY=${MISSING_VAR:-default_key}").unwrap();

        let config = Config::from_file(file.path().to_str().unwrap()).unwrap();
        assert_eq!(config.get("DATABASE_URL").unwrap(), "postgres://localhost:5432/mydb");
        assert_eq!(config.get("API_KEY").unwrap(), "${MISSING_VAR:-default_key}");
    }
